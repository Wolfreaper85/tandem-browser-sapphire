  ...
  const ocSidebar = (() => {
    ...
    function TOKEN() { return window.__TANDEM_TOKEN__ || ''; }
    ...
    async function loadConfig() {
      ...
      config = data.config;
      config.items = (config.items || []).filter(i => i.id !== 'news'); // remove deprecated Personal News
    ...
    function render() {
      ...
      // Home button (navigates active tab to DuckDuckGo)
      const homeHtml = `
        <button class="sidebar-item" id="sidebar-home-btn" title="Home – DuckDuckGo">
      ...
      // 4 sections: home / workspace icons / communication / utilities, with separators
      const wsEnabled = config.items.find(i => i.id === 'workspaces');
      const wsHtml = (wsEnabled && wsEnabled.enabled !== false) ? renderWorkspaceIcons() : '';
      itemsEl.innerHTML =
        homeHtml +
    ...
    async function loadHistoryPanel() {
      ...
      safeSetPanelHTML(`
        <div class="history-panel">
          ...
          <div style="padding:8px 12px; border-top:1px solid rgba(255,255,255,0.08);">
            <button id="history-clear-btn" style="width:100%; padding:6px 0; background:rgba(239,68,68,0.15); color:#f87171; border:1px solid rgba(239,68,68,0.3); border-radius:6px; cursor:pointer; font-size:12px;">Clear All History</button>
          </div>
        </div>`);
      ...
      document.getElementById('history-clear-btn')?.addEventListener('click', async () => {
        ...
        try {
          await fetch('http://localhost:8765/history/clear', {
            method: 'DELETE', headers: { Authorization: `Bearer ${TOKEN()}` }
          });
          const listEl = document.getElementById('history-list');
          if (listEl) listEl.innerHTML = '<div class="bm-empty">No history</div>';
    ...
    async function loadDownloadsPanel() {
      ...
      try {
        ...
        if (listEl) {
          if (!downloads.length) {
            listEl.innerHTML = '<div class="bm-empty">No downloads</div>';
          } else {
            listEl.innerHTML = downloads.map(dl => {
              const statusIcon = dl.status === 'completed' ? '\u2705' :
                                 dl.status === 'progressing' ? '\u23f3' :
                                 dl.status === 'cancelled' ? '\u274c' : '\u26a0\ufe0f';
              const size = dl.totalBytes > 0 ? formatBytes(dl.totalBytes) : '';
        ...
        const listEl = document.getElementById('downloads-list');
        if (listEl) listEl.innerHTML = '<div class="bm-empty">Failed to load downloads</div>';
      }
    }
    ...
    const SETUP_SECTIONS = [
      ...
      { title: 'Browser Utilities', ids: ['pinboards','bookmarks','history','downloads'] },
    ];
    ...
    function init() {
      ...
      document.getElementById('sidebar-items').addEventListener('click', e => {
        // Handle home button — navigate active tab to DuckDuckGo
        const homeBtn = e.target.closest('#sidebar-home-btn');
        if (homeBtn) {
          fetch('http://127.0.0.1:8765/navigate', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${TOKEN()}` },
            body: JSON.stringify({ url: 'https://duckduckgo.com' })
          });
          return;
        }
