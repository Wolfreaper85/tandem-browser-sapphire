(() => {
    ...
    const chatRouter = (() => {
      ...
      // Dynamic display names — persona name from Sapphire, updated periodically
      let _sapphirePersonaName = localStorage.getItem('sapphire_persona_name') || 'Sapphire';
      let _sapphireUserName = localStorage.getItem('sapphire_user_name') || 'You';

      // Read persona name from ~/.tandem/sapphire-persona.json (written by bridge)
      async function _updateSapphireNames() {
        try {
          if (typeof require !== 'undefined') {
            const fs = require('fs');
            const path = require('path');
            const os = require('os');
            const filePath = path.join(os.homedir(), '.tandem', 'sapphire-persona.json');
            if (fs.existsSync(filePath)) {
              const data = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
              if (data.persona) { _sapphirePersonaName = data.persona; localStorage.setItem('sapphire_persona_name', data.persona); }
              if (data.username) { _sapphireUserName = data.username; localStorage.setItem('sapphire_user_name', data.username); }
            }
          }
        } catch (e) { /* ignore */ }
      }
      _updateSapphireNames();
      setInterval(_updateSapphireNames, 30000);
      ...
      function appendMessage(role, text, timestamp, source, image) {
        ...
        if (role === 'user') {
          cls = 'robin';
          const activeId = (typeof router !== 'undefined' && router.getActiveId) ? router.getActiveId() : null;
          name = (activeId === 'sapphire') ? _sapphireUserName : 'Robin';
        } else if (sourceClass === 'claude') {
          cls = 'claude';
          name = 'Claude';
        } else if (sourceClass === 'sapphire') {
          cls = 'wingman';
          name = _sapphirePersonaName;
        } else {
          cls = 'wingman';
          name = 'Wingman';
        }
      ...
      const sapphireBackend = (typeof SapphireBackend !== 'undefined') ? new SapphireBackend() : null;
      if (sapphireBackend) router.register(sapphireBackend);
      ...
      const btnSA = document.getElementById('btn-backend-sapphire');
      ...
      const dotSA = document.getElementById('dot-sapphire');
      ...
      function updateBackendUI(activeId) {
        ...
        if (btnSA) btnSA.classList.toggle('active', activeId === 'sapphire');
        ...
        if (activeId === 'both') {
          ...
          if (activeId === 'sapphire') {
            inputEl.placeholder = 'Message to Sapphire...';
        ...
        if (typingText) {
          ...
          } else if (activeId === 'sapphire') {
            typingText.textContent = 'Sapphire is thinking...';
          } else if (activeId === 'claude') {
      ...
      if (btnSA) btnSA.addEventListener('click', () => switchBackend('sapphire'));
      ...
      router.onConnectionChange((connected, backendId) => {
        ...
        } else if (backendId === 'sapphire') {
          if (dotSA) dotSA.style.background = connected ? 'var(--success)' : 'var(--accent)';
      ...
      async function sendMessage() {
        ...
        if (currentMode === 'both') {
          ...
          // Sapphire: send via IPC→panelManager→bridge
          if (activeId === 'sapphire') {
            const robinMsg = appendMessage('user', text, Date.now(), 'robin');
            robinMsg.dataset.localMessage = 'true';
            window.tandem?.sendChatMessage(text);
          } else if (activeId === 'openclaw') {
            const robinMsg = appendMessage('user', text, Date.now(), 'robin');
            robinMsg.dataset.localMessage = 'true';
      ...
      // Force Sapphire as the only backend (tabs hidden)
      router.connectAll();
      switchBackend('sapphire');
      ...
      if (window.tandem && window.tandem.onChatMessage) {
        window.tandem.onChatMessage((msg) => {
          ...
          // When Sapphire backend is active, tag wingman messages as 'sapphire'
          // so appendMessage uses the persona name instead of generic "Wingman"
          const activeId = router.getActiveId();
          const source = (activeId === 'sapphire' && msg.from === 'wingman') ? 'sapphire' : msg.from;
